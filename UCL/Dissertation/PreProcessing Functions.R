...
data_transform<-function(mat, transform_codes=NULL){
  ...
  transform_codes[1]<-0 #if transform codes are not NULL skip the above, first value of transform codes = 1 (Time/Date untransformed), 
  for(i in 1:ncol(mat)){ #for each variable in the data matrix
    mat[,i]<-individual_transform(column=mat[, i], code=transform_codes[i]) #column = loop over the variables in the data
  }
  mat["Time"] = time
  return(mat)
}
...
#' @export
clean_data<-function(raw_data, slow_names, FFR_name, fast_names, transform_codes=NULL){
  manually_transformed<-data_transform(mat = raw_data, transform_codes = transform_codes)
  ...
  data_all_dates<-data.frame(cbind(Time,manually_transformed[,indexes]), lag_FFR=dplyr::lag(FFR))
  data_all_dates$Time = as.Date(data_all_dates$Time)
  start<-which(data_all_dates$Time==raw_data[1,1])
  end<-which(data_all_dates$Time==raw_data[nrow(raw_data),1])
  data_in_window<-data_all_dates[start:end,]
  
  delete<-NULL
  for(i in 1:nrow(data_in_window)){ #removing heading NAs
  ...
  for(i in nrow(data_in_window):1){ #removing tail NAs
    if(sum(anyNA(data_in_window[i,]))){
      delete<-c(delete, i)
    }else{
      break
