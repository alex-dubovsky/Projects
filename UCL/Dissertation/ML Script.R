...
dml_lp <- function(dt, y_var, shock_var,
                   H = 20, lags = 3, K = 15, buffer_folds = 1,
                   # θ(h) HAC settings
                   ...
                   m_asw_c_pi   = 0.5,  m_asw_alpha = 0.05, m_asw_B = 1000, m_asw_full_Omega = FALSE, m_asw_aggL = "p95",
                   m_ah_C_PI    = 0.5,  m_ah_q_fixed = 8,   m_ah_iters = 5,   m_ah_kernel_phi = "uniform",
                   scale_outcome_m = FALSE,
                   # g(X) settings
                   nuisance_g   = c("ahrens","asw"),
                   g_asw_c_pi   = 0.5,  g_asw_alpha = 0.05, g_asw_B = 1000, g_asw_full_Omega = FALSE, g_asw_aggL = "p95",
...
#' @export
dml_application <- function(dt, y_var, shock_var,
                            H = 20, lags = 13, K = 15, buffer_folds = 1,
                            scale_outcome_m = FALSE,
                            scale_outcome_g = FALSE,
                            predetermined_y = TRUE,
                            cumulate = TRUE,
  ...
  M_X <- cf_predict(X = X_mat_h, y = D_all, folds = folds,
                    method = "asw",
                    asw_Q_T = Q_T_m, asw_c_pi = 0.4, # DML_2
                    asw_alpha = 0.05, asw_B = 1000, asw_full_Omega = FALSE,
                    scale_outcome = scale_outcome_m, mu = d_mu, sdv = d_sd)
  m_hat  <- M_X$pred
  D_orth <- D_all - m_hat
  ...
  score_mat <- matrix(0, nrow = Tn, ncol = H + 1)  # store scores across horizons
  ...
  for (h in 0:H) {
    ...
    Yh <- DATA[[if (h == 0) y_var else paste0(y_var, "_lead", h)]]
    ...
    score_t <- psi_b_t + theta_hat[h + 1] * psi_a_t
    score_mat[, h + 1] <- score_t / (-psi_a)   # normalized score for covariance
  }
  
  # ---------- HAC variance estimation ----------
  if (!cumulate) {
    ci_lo <- ci_hi <- numeric(H + 1)
    for (h in 0:H) {
      score_t <- score_mat[, h + 1]
      Lh <- if (bw == "fixed") nw_lag_default(Tn) else L_eq64_bartlett_vec(score_t)
      Om <- hac_lrv_matrix_kernel(matrix(score_t, ncol = 1), L = Lh,
                                  kernel = kernel, finite_adjust = FALSE)
      se_h <- sqrt(Om[1,1] / Tn)
      zc   <- qnorm(0.975)
      ci_lo[h + 1] <- theta_hat[h + 1] - zc * se_h
      ci_hi[h + 1] <- theta_hat[h + 1] + zc * se_h
    }
    
  } else {
    # cumulative case
    theta_hat <- cumsum(theta_hat)
    ci_lo <- ci_hi <- numeric(H + 1)
    
    # HAC covariance across horizons
    Lh <- if (bw == "fixed") nw_lag_default(Tn) else L_eq64_bartlett_vec(score_t)
    Om <- hac_lrv_matrix_kernel(score_mat, L = Lh, kernel = kernel, finite_adjust = FALSE)
    
    for (h in 0:H) {
      idx <- 1:(h+1)
      one_vec <- rep(1, length(idx))
      var_cum <- t(one_vec) %*% Om[idx, idx] %*% one_vec / Tn
      se_cum  <- sqrt(var_cum)
      zc      <- qnorm(0.975)  # asymptotic normal
      ci_lo[h + 1] <- theta_hat[h + 1] - zc * se_cum
      ci_hi[h + 1] <- theta_hat[h + 1] + zc * se_cum
    }
  }
...
################################################################################
# Safe bandwidth selector
################################################################################
################################################################################
# Safe bandwidth selector
################################################################################
safe_bandwidth <- function(s, Tn) {
  Lh <- suppressWarnings(L_eq64_bartlett_vec(s))
  if (!is.finite(Lh) || Lh < 0) {
    Lh <- nw_lag_default(Tn)   # fallback NW default
  }
  return(as.integer(Lh))
}

################################################################################
# Build design matrix with slow/fast variables
################################################################################
################################################################################
# Build design matrix with slow/fast variables (avoid double inclusion of shock)
################################################################################
build_matrix_slowfast <- function(data, slow_vars, fast_vars, y_var, shock_var,
                                  H = 20, lags = 13) {
  # --- add y and shock safely (avoid duplicates) ---
  if (y_var == shock_var) {
    dt <- data.table::as.data.table(
      cbind(slow_vars, fast_vars,
            y = data[[y_var]])
    )
    setnames(dt, "y", y_var)
  } else {
  ...
  # ---- Drop contemporaneous fast vars (keep their lags only) ----
  fast_names <- colnames(fast_vars)
  dt[, (fast_names) := NULL]
...
################################################################################
# DML with slow/fast separation (robust bandwidths)
################################################################################
################################################################################
# Build design matrix with slow/fast variables
################################################################################
build_matrix_slowfast <- function(data, slow_vars, fast_vars, y_var, shock_var,
  ...
  # ---- Drop contemporaneous fast vars (keep only their lags) ----
  fast_names <- colnames(fast_vars)
  dt[, (fast_names) := NULL]
...
################################################################################
# DML with slow/fast separation (ASW only)
################################################################################
dml_application_slowfast <- function(data, slow_vars, fast_vars,
                                     y_var, shock_var,
                                     H = 20, lags = 13, K = 15, buffer_folds = 1,
                                     scale_outcome_m = FALSE,
                                     scale_outcome_g = FALSE,
  ...
  # ---- Build design matrix ----
  DATA <- build_matrix_slowfast(data, slow_vars, fast_vars,
                                y_var, shock_var, H, lags)
  all_names <- names(DATA)
  lead_cols <- grep(paste0("^", y_var, "_lead"), all_names, value = TRUE)
  ...
  # ---- Regressor matrix: exclude leads & contemporaneous shock ----
  exclude  <- c(lead_cols, shock_var)
  exclude0 <- c(exclude, y_var)
  X_mat_h  <- as.matrix(DATA[, setdiff(all_names, exclude),  with = FALSE])
  ...
  # ---------- m(X) ----------
  d_mu <- if (scale_outcome_m) mean(D_all) else 0
  d_sd <- if (scale_outcome_m) sd(D_all)   else 1
  if (!is.finite(d_sd) || d_sd == 0) d_sd <- 1
  
  Q_T_m <- compute_QT_bandwidth(X_mat_h, D_all, mu = d_mu, sdv = d_sd, aggL = "p95")
  M_X <- cf_predict(X = X_mat_h, y = D_all, folds = folds,
                    method = "asw",
                    asw_Q_T = Q_T_m, asw_c_pi = 0.4,
                    asw_alpha = 0.05, asw_B = 1000, asw_full_Omega = FALSE,
                    scale_outcome = scale_outcome_m, mu = d_mu, sdv = d_sd)
  ...
  # ---------- g(X) ----------
  g_mu0 <- if (scale_outcome_g) mean(Y0) else 0
  g_sd0 <- if (scale_outcome_g) sd(Y0)   else 1
  if (!is.finite(g_sd0) || g_sd0 == 0) g_sd0 <- 1
  
  Q_T_g <- compute_QT_bandwidth(X_mat_h, Y0, mu = g_mu0, sdv = g_sd0, aggL = "p95")
  
  theta_hat <- numeric(H + 1)
  score_mat <- matrix(0, nrow = Tn, ncol = H + 1)
  
  for (h in 0:H) {
    if (predetermined_y && h == 0) {
      theta_hat[1] <- 1   # Normalization: shock impacts itself
      score_mat[,1] <- 0
      next
    }
    ...
    G_X <- cf_predict(X = X_mat_h, y = Yh, folds = folds,
                      ...
                      asw_Q_T = Q_T_g, asw_c_pi = 0.4,
                      asw_alpha = 0.05, asw_B = 1000, asw_full_Omega = FALSE,
                      scale_outcome = scale_outcome_g, mu = g_mu0, sdv = g_sd0)
    g_hat <- G_X$pred
  ...
  if (!cumulate) {
    ...
    theta_hat <- cumsum(theta_hat)
    Lh <- if (bw == "fixed") nw_lag_default(Tn) else L_eq64_bartlett_vec(score_mat[,1])
    Om <- hac_lrv_matrix_kernel(score_mat, L = Lh, kernel = kernel, finite_adjust = FALSE)
...
dml_lp_final <- function(dt, slow_vars, fast_vars, y_var, shock_var,
                   H = 20, lags = 3, K = 15, buffer_folds = 1,
                   # θ(h) HAC settings
                   theta_kernel = c("bartlett","cosine","uniform"),
                   theta_bw     = c("fixed","andrews_ar1"),
                   ...
                   # m(X) settings (choose method separately)
                   nuisance_m   = c("asw","ahrens"),
                   m_asw_c_pi   = 0.0,  m_asw_alpha = 0.05, m_asw_B = 1000, m_asw_full_Omega = FALSE, m_asw_aggL = "p95",
                   m_ah_C_PI    = 0.5,  m_ah_q_fixed = 8,   m_ah_iters = 5,   m_ah_kernel_phi = "uniform",
                   scale_outcome_m = FALSE,
                   # g(X) settings
                   nuisance_g   = c("ahrens","asw"),
                   g_asw_c_pi   = 0.0,  g_asw_alpha = 0.05, g_asw_B = 1000, g_asw_full_Omega = FALSE, g_asw_aggL = "p95",
                   g_ah_C_PI    = 0.5,  g_ah_q_fixed = 8,   g_ah_iters = 5,   g_ah_kernel_phi = "uniform",
                   scale_outcome_g = FALSE,
                   # identification
                   predetermined_y = TRUE
  ...
  ##############################################################################
  # --- Recursive ordering: keep contemporaneous slow vars, but only lagged fast vars
  ##############################################################################
  DATA <- data.table::as.data.table(
    cbind(slow_vars, fast_vars, y = dt[[y_var]], shock = dt[[shock_var]])
  )
  setnames(DATA, "y", y_var)
  setnames(DATA, "shock", shock_var)
  
  cols <- names(DATA)
  for (cl in cols) {
    for (L in seq_len(lags)) {
      nm <- paste0(cl, "_lag", L)
      DATA[, (nm) := data.table::shift(get(cl), n = L, type = "lag")]
    }
  }
  ...
  for (L in seq_len(H)) {
    nm <- paste0(y_var, "_lead", L)
    DATA[, (nm) := data.table::shift(y0, n = L, type = "lead")]
  }
  
  # drop contemporaneous fast vars (keep only their lags)
  fast_names <- colnames(fast_vars)
  DATA[, (fast_names) := NULL]
  ...
  # Exclude leads & contemporaneous shock
  exclude  <- c(lead_cols, shock_var)
  exclude0 <- c(exclude, y_var)
  X_mat_h  <- as.matrix(DATA[, setdiff(all_names, exclude),  with = FALSE])
  ...
  list(
    ...
    diagnostics = list(theta = diag_theta, m = M_X$diag)
  )
}
